// 方式1 编辑器vscode不报错，但是creator里面会报错且运行出错（allocUnsafe找不到，推测是由于buffer包内用了BigInt导致实际import失败）
import { Buffer } from './Buffer'; 

// 方式2 编辑器vscode报错，但creator能正常运行！！！
// import b from "./buffer.js"; 
// const { Buffer } = b;

import { utf8_read } from './Utf8';
// 注： luban使用nodejs自身的buffer包，但很多函数不能用在cocos平台，故这里该使用buffer这个第三方同名包，且需要改造如BigInt等
...
export class ByteBuf {
    private static emptyBuff = Buffer.allocUnsafe(0)
    ...
    getBytesNotSafe(): any { return this._bytes }
    ...
    private static emptyBytes: Uint8Array = new Uint8Array(0) //无参版本es2017+才支持
    ...
    private static PropSize(initSize: number, needSize: number): number|undefined {
    ...
    /* lxm: 这里对比protobuf速度差很多，pbjs源码在src/writer.js下如
    ...
    */
    ...
    //     // if (n > 0) {
    //     //     this.EnsureWrite(n)
    //     //     this._bytes.fill(x, this._writerIndex, this._writerIndex + n/*, 'utf-8'*/) // nodejs内置版本才有这个参数
    //     //     this._writerIndex += n
    //     // }

    //     const n = utf8.length(x)
    //     this.WriteSize(n)
    //     if (n > 0) {
    //         this.EnsureWrite(n)
    //         utf8.write(x, this._bytes, this._writerIndex)
    //         this._writerIndex += n
    //     }
    // }
    ...
    ReadString(): string {
        ...
        if (n > 0) {
            ...
            const s = utf8_read(this._bytes, this._readerIndex, this._readerIndex+n)
            this._readerIndex += n
            return s
        }
        else {
